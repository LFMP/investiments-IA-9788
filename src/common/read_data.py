            ...
            for (index, row) in enumerate(spamreader):
                if(index == 0):
                    ...
                    for column_name in header:
                        data[filename[:-18]][column_name] = []
                else:
                    for (i, value) in enumerate(row):
                        if (i != 0):
                            data[filename[:-18]][header[i]].append(float(value))
                        else:
                            data[filename[:-18]][header[i]].append(value)
        for name in header:
            data[filename[:-18]][name] = np.array(
                data[filename[:-18]][name]).flatten()
    return data
